 ...
 */
JobGenerationController::JobGenerationController(
    const std::string& hostname, int num_jobs,
...
int JobGenerationController::main()
{
  ...
  while (num_completed_jobs < _num_jobs) {
    ...
    if (std::dynamic_pointer_cast<TimerEvent>(event)) {
      ...
      auto target_batch_service_controller      = _batch_service_controllers.at(target_bath_service_controller_index);
